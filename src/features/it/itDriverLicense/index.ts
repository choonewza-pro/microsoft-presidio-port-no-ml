/**
 * Italy Driver License Recognizer
 * Ported from presidio-analyzer/presidio_analyzer/predefined_recognizers/country_specific/italy/it_driver_license_recognizer.py
 */
import type { RecognizerResult } from "../../../core/types.ts";

export const ENTITY_TYPE = "IT_DRIVER_LICENSE" as const;
export const COUNTRY_CODE = "it" as const;
export const SUPPORTED_LANGUAGE = "it" as const;
export const BASE_SCORE = 0.2;

export const PATTERN_SOURCE = "(([A-Z]{2}\\d{7}[A-Z])|(U1[BCDEFGHLJKMNPRSTUWYXZ0-9]{7}[A-Z]))";
export const REGEX = new RegExp(`\\b${PATTERN_SOURCE}\\b`, "gi");
export const REGEX_SINGLE = new RegExp(`^${PATTERN_SOURCE}$`, "i");
export const PATTERNS = [{ name: "Driver License", regex: PATTERN_SOURCE, score: BASE_SCORE }];

export const CONTEXT: readonly string[] = ["patente", "patente di guida", "licenza", "licenza di guida"] as const;
...
export function analyze(text: string): RecognizerResult[] {
  ...
  for (const m of text.matchAll(re)) {
    ...
    results.push({
      ...
      recognitionMetadata: { recognizerName: "ItDriverLicenseRecognizer" },
      analysisExplanation: {
        recognizer: "ItDriverLicenseRecognizer",
        patternName: "Driver License",
        pattern: REGEX.source,
        originalScore: BASE_SCORE,
        validationResult: null,
        textualExplanation: "Detected by `ItDriverLicenseRecognizer` using pattern `Driver License`",
      },
    });
  }
  return results.sort((a, b) => b.score - a.score || a.start - b.start);
...
export class ItDriverLicenseRecognizer {
