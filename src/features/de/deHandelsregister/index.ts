/**
 * Germany Handelsregister Recognizer
 * Ported from presidio-analyzer/presidio_analyzer/predefined_recognizers/country_specific/germany/de_handelsregister_recognizer.py
 * HR[AB] optional space 1-6 digits
 */
import type { RecognizerResult } from "../../../core/types.ts";

export const ENTITY_TYPE = "DE_HANDELSREGISTER" as const;
export const COUNTRY_CODE = "de" as const;
export const SUPPORTED_LANGUAGE = "de" as const;
export const BASE_SCORE = 0.5;

export const PATTERN_SOURCE = "HR[AB]\\s*\\d{1,6}";
export const REGEX = new RegExp(`\\b${PATTERN_SOURCE}\\b`, "g");
export const REGEX_SINGLE = new RegExp(`^${PATTERN_SOURCE}$`);
export const PATTERNS = [{ name: "Handelsregisternummer HRA/HRB", regex: PATTERN_SOURCE, score: BASE_SCORE }];
...
export function validateResult(patternText: string): boolean | null { return null; }
...
export function analyze(text: string): RecognizerResult[] {
  ...
  for (const m of text.matchAll(re)) {
    ...
    results.push({
      ...
      recognitionMetadata: { recognizerName: "DeHandelsregisterRecognizer" },
      analysisExplanation: {
        recognizer: "DeHandelsregisterRecognizer",
        patternName: "Handelsregisternummer HRA/HRB",
        pattern: REGEX.source,
        originalScore: BASE_SCORE,
        validationResult: null,
        textualExplanation: "Detected by `DeHandelsregisterRecognizer` using pattern `Handelsregisternummer HRA/HRB`",
      },
    });
  }
  return results.sort((a, b) => b.score - a.score || a.start - b.start);
...
export class DeHandelsregisterRecognizer {
